...
function initPage(){ 
    hom = document.getElementById("cos_hom");
	hom.onclick = disp_cont_hom;
	
	format = document.getElementById("cos_format");
	format.onclick = disp_cont_format;
	
	contact = document.getElementById("cos_contact");
...
function display(){
     if(req.readyState==4){
	    if(req.status==200){
		     document.getElementById("coscontent").innerHTML = req.responseText;
		}
	}
}
